...
function HabitBlock({
  ...
  unitLabel: string;
  incLabel: string;
  messageFn: (n: number) => CoachMessage;
  positive?: boolean;
}) {
  const { c, brand } = useTheme();
  const qc = useQueryClient();
  const today = useQuery({
    queryKey: ['habit', kind, 'today'],
    queryFn: () => habitCountToday(kind),
  });
  const week = useQuery({ queryKey: ['habit', kind, '7d'], queryFn: () => habitLast7Days(kind) });
  const streak = useQuery({
    queryKey: ['habit', kind, 'streak'],
    queryFn: () => cleanStreak(kind),
  });
  ...
  return (
    <Card>
      ...
      <View style={{ alignItems: 'center', marginVertical: 8 }}>
        ...
        <AnimatedNumber
          value={currentCount}
          style={{ fontSize: 48, fontWeight: '800', color: c.text, letterSpacing: -1 }}
        />
      </View>
      <View
        style={{
  ...
  return (
    <Screen>
      ...
      <HabitBlock
        ...
        incLabel="+1 cigarette"
        messageFn={cigaretteMessage}
      />
      <HabitBlock
        kind="alcohol"
        title="🍺 Alcohol"
